...
service="bstoragemanager_1"
nodes = getNode(service)

for node in nodes:
    ssh.connect(hostname=node,username=user,password=passwd)
    pid=getPid(node,service)
    print "Before kill, %s on %s pid is %s"%(service,node,ingreen(pid))
    killSer(node,pid)
    ssh.close()
